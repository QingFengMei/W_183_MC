...
%% Scenarios
sc1 = "GI = CC";
sc1 = sc1 + newline + "LV = NC";
sc1 = sc1 + newline + "pE = CC";
...
% sub1 = subplot(3,3,1);
axes(p(1));
plot(Y1,YY1,'.', MarkerSize=0.5)
% axis square
hold on
% yline([0],'r',LineWidth=1.5)
ylim([-5,30]);
t1 = text(0.98,0.98,'(a)','Units','normalized', 'fontsize',12,'HorizontalAlignment','right',VerticalAlignment='top');
t1_1 = text(0.1,0.90,sc1,'Units','normalized',...
    'fontsize',10, VerticalAlignment='top',HorizontalAlignment='left',Fontsize=8);
xlim([-10,25]);
xticklabels({'0','10','20'});
xticks([0,10,20]);
...
xlabel('Predicted \Delta^{95}Mo of the BSE',FontSize=12)
xlim([-10,25]);
xticklabels({'0','10','20'});
xticks([0,10,20]);
yticklabels({'0','10','20','30'})
...
for i = 1:9;
axes(p(i));
re=rectangle('Position',[2,-5,10,10],'EdgeColor','r',LineWidth=1.5);
end
